...
class ProductionRepositoryTest {
    ...
    private class FakeClient(
        ...
        override suspend fun getBatchInverterEnergy(date: String): BatchEnergyFetch = BatchEnergyFetch(ApiResult.ConfigurationError)
    }

    private fun successFetch(power: Int = 8000) =
        ProductionFetch(
    ...
    @Test
    fun newInstanceWithinThrottle_showsLastPersistedSnapshot() =
        runBlocking {
    ...
    @Test
    fun fetchesAgainAfterWindowElapses() =
        runBlocking {
            val client = FakeClient(successFetch())
            val repository = repo(client)
            repository.refresh()
    ...
    @Test
    fun networkError_setsBannerAndKeepsCachedSnapshot() =
        runBlocking {
            val client = FakeClient(successFetch())
            val repository = repo(client)
    ...
    @Test
    fun apiError_setsBannerAndKeepsCachedSnapshot() =
        runBlocking {
            val client = FakeClient(successFetch())
            val repository = repo(client)
            repository.refresh() // success, cached
    ...
    @Test
    fun success_clearsBannerAfterError() =
        runBlocking {
    ...
    @Test
    fun configurationError_doesNotCountOrLogOrThrottle() =
        runBlocking {
    ...
    @Test
    fun networkError_isLoggedButNotCountedAndDoesNotThrottle() =
        runBlocking {
            val client = FakeClient(ProductionFetch(ApiResult.NetworkError, "/ecu/energy", 1L, "GET", ""))
            repo(client).refresh()
    ...
    @Test
    fun networkError_doesNotBlockNextAttempt() =
        runBlocking {
            val client = FakeClient(ProductionFetch(ApiResult.NetworkError, "/ecu/energy", 1L, "GET", ""))
            val repository = repo(client)
    ...
    @Test
    fun success_persistsValueTimestampForNextInstance() =
        runBlocking {
    ...
    @Test
    fun errorAndValuePersistViaCurrentState() =
        runBlocking {
    ...
    @Test
    fun success_clearsPersistedErrorForNextInstance() =
        runBlocking {
            repo(FakeClient(ProductionFetch(ApiResult.ApiError(code = 4000), "/e", 1L, "GET", "{}"))).refresh()
            now += 11 * 60 * 1000L
    ...
    @Test
    fun apiError_isNotCountedDoesNotThrottleAndRetries() =
        runBlocking {
    ...
    @Test
    fun authError_isClassifiedAndNotCounted() =
        runBlocking {
    ...
    @Test
    fun onlySuccessCounts_failureThenSuccess() =
        runBlocking {
